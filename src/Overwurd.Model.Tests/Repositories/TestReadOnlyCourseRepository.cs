...
namespace Overwurd.Model.Tests.Repositories
{
    ...
    {
        private async Task<User> CreateAndStoreUserAsync(string userName)
        {
        ...
        {
            var user = await CreateAndStoreUserAsync("Test User");
        ...
        {
            var user1 = await CreateAndStoreUserAsync("Test User 1");
            var course1 = new Course(name: "Course 1", description: "Description 1") { User = user1 };

            var user2 = await CreateAndStoreUserAsync("Test User 2");
